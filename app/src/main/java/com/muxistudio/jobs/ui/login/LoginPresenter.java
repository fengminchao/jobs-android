...
@PerActivity
public class LoginPresenter extends SubscriptionPresenter
    ...
    @Inject
    public LoginPresenter(UserApi userApi, UserStorge userStorge) {
    ...
    private void insertUserIfNotExist(UserInfo data) {
        List<UserInfo> userInfos = mUserInfoDao.queryBuilder().where(
                UserInfoDao.Properties.Mail.eq(data.getMail())).list();
    ...
    private void insertOrUpdateUser(User user) {
        List<User> users = mUserDao.queryBuilder().where(
                UserDao.Properties.Mail.eq(user.getMail())).list();
        if (users.isEmpty()) {
