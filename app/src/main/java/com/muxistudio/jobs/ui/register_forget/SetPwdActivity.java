...
public class SetPwdActivity extends ToolbarActivity implements SetPwdContract.View {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == R.id.action_set) {
            if (action == ACTION_REGISTER) {
                mSetPwdPresenter.register(mEditMail.getText().toString(),
                        mEditPwd.getText().toString(),
                        mEtAuth.getText().toString());
            } else {
                mSetPwdPresenter.resetPwd(mEditMail.getText().toString(),
                        mEditPwd.getText().toString(),
                        mEtAuth.getText().toString());
            }
            return true;
    ...
    @Override
    public void showMailError(String msg) {
        mLayoutName.setErrorEnabled(true);
        mLayoutName.setError(msg);
    }

    @Override
    public void showPwdError(String msg) {
        mLayoutPwd.setErrorEnabled(true);
        mLayoutPwd.setError(msg);
    }

    @Override
    public void showAuthCodeError(String msg) {
