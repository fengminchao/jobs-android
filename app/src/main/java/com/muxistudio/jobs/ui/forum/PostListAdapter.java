...
public class PostListAdapter extends RecyclerView.Adapter<PostListAdapter.ViewHolder> {
  ...
  @Override public void onBindViewHolder(ViewHolder holder, int position) {
    ...
    if (TextUtils.isEmpty(mPostDataList.get(position).avator)){
      Picasso.with(mContext).load(R.drawable.default_avator).transform(new CircleTransformation()).into(holder.mIvAvator);
    }else {
      Picasso.with(mContext).load(mPostDataList.get(position).avator).transform(new CircleTransformation()).into(holder.mIvAvator);
    }
